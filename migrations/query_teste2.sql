...
-- ============================================================
-- Separar em queries executáveis individualmente
...
categoria_vendas_elite AS (
    ...
    FROM vendas v
    INNER JOIN top_10_ids t ON v.id_client = t.id_client
    LEFT JOIN categorias_normalizadas cn ON v.id_product = cn.code
...
SELECT
    ...
    faturamento_total,
    numero_transacoes,
    ROUND(quantidade_total * 100.0 / (SELECT SUM(quantidade_total) FROM categoria_vendas_elite), 2) AS percentual
